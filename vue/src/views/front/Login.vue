<template>
 <div class="wrapper-front">
   <div style="margin: 200px auto; width: 350px; height: 300px; padding: 20px; border-radius: 10px" class="login_form">
     <div style="margin: 20px 0; text-align: center; font-size: 24px; font-weight: bold; color: #B088FF"> Login </div>
    ...
    login() {
      ...
      this.$refs['userForm'].validate((valid) => {
        if (valid) {
          // 表单验证合法
          // 获取后端数据判断跳转
          this.request.post("/stdUser/login", this.stdUser).then(res => {
            if(res.code === '200') {
              // 存储用户信息到浏览器
              localStorage.setItem("stdUser", JSON.stringify(res.data))
              // 登录成功: 延时跳转
              setTimeout(() => {
